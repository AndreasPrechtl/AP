...
public class FreezableQueue<T> : ExtendableQueue<T>, IFreezable
{
    ...
    public new FreezableQueue<T> Clone() => (FreezableQueue<T>)this.OnClone();

    protected override CollectionBase<T> OnClone() => new FreezableQueue<T>(this, this.Comparer, this.IsFrozen);
