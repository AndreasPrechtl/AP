...
namespace AP.UniformIdentifiers
{
    ...
    public class Unc : UrlBase, IFileUri, IAbsoluteOrRelativeUri
    {
        ...
        private static readonly char[] _reservedCharacters = new[] { '*', ':', '?', '"', '/', '\\', '<', '>', '|', '\0', '\t', '\r', '\n', '\b' };
        private readonly Deferrable<Unc> _parent;
        
        public static char[] ReservedCharacters
        {
            get { return (char[])_reservedCharacters.Clone(); }
        }
            ...
            : this()
        {
            ...
            Collections.List<string> parts = new Collections.List<string>();
            ...
            foreach (string clean in cleaned)
            {
                ...
                if (!segment.IsEmpty())
                {
                    if (isRemote)
                    {
                        ...
                        if (i == start)
                        {
                            bool extractedIsSecure;
                            ushort extractedPort;
                            Host extractedHost = ExtractAuthority(segment, out extractedIsSecure, out extractedPort);
        ...
        private static Host ExtractAuthority(string extendedHost, out bool isSecure, out ushort port)
        {
            ...
            if (lastAtIndex > -1)
            {
                ...
                string[] splitted = extendedHost.Split('@');

                if (splitted.Length > 1)
                {
                    for (int i = 1; i < 3; i++)
                    {
                        string current = splitted[splitted.Length - i];
                ...
                StringBuilder sb = new StringBuilder(extendedHost);
