...
using AP.ComponentModel.Validation;
using AP.Linq;
...
namespace AP.Data
{
        ...
        where TEntity : class
    {
        ...
        public AP.Collections.IList<TEntity> Update(Expression<Predicate<TEntity>> where, Action<TEntity> action)
        {
            this.ThrowIfContextIsDisposed();
        ...
        public AP.Collections.IList<TEntity> Delete(Expression<Predicate<TEntity>> where)
        {
            this.ThrowIfContextIsDisposed();
            var q = this.Query().Where(where.Cast<Func<TEntity, bool>>());
