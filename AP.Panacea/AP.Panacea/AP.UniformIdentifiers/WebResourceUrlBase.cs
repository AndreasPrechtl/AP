...
namespace AP.UniformIdentifiers
{
    ...
    public abstract class WebResourceUrlBase : UrlBase, IUriWithUserName, IPasswordProtectableUri, IPortUsingUri, IFileUri, IQueryableUri, IFragmentableUri
    {
            ...
            : this()
        {
            ...
            // lets see if a host is needed - if one is needed I need to make sure that it stays within certain
            ...
            if (!sb.IsNullOrEmpty() && (sb[0] != '?' && sb[0] != '#'))
            {
                ...
                StringBuilder pathBuilder = new StringBuilder(parsedPath.Length);

                char separator = '/';
                string[] cleanup = parsedPath.Split(new[] { separator }, StringSplitOptions.RemoveEmptyEntries);
        ...
        protected override void BuildFullName(ref StringBuilder builder)
        {
            ...
            if (!this.Path.IsNullOrEmpty())
                builder.Append(Uri.EscapeUriString(this.Path));
        ...
        public virtual ushort DefaultPort
        {
            get { return 80; }
        }
        ...
        public WebResourceUrlBase Parent
        {
            get { return _parent.Value; }
        }
